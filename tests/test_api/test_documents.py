...
def test_create(client: CouchClient, async_run: Callable):
    response = async_run(client.doc_create_or_update(db_name, doc_name, dict(val=1)))
    assert response.status_code == 201
    assert response.model.id == doc_name
    assert response.model.rev is not None

    response = async_run(
        client.doc_create_or_update(
...
def test_create_with_attachments(client: CouchClient, async_run: Callable):
    ...
    response = async_run(
        client.doc_get(db_name, doc_with_attachments, attachments=True)
    )
    assert response.status_code == 200
...
def test_copy(client: CouchClient, async_run: Callable):
    ...
    response = async_run(
        client.doc_copy(db_name, doc_name, "test_copy", rev=original.model._rev)
    )
    assert response.model.ok is True
    assert response.status_code == 201
    ...
    response = async_run(client.doc_copy(db_name, doc_name, "test_copy2", batch="ok"))
    assert response.model.ok is True
    # todo: returns 201 rather then 202
    # assert response.status_code == 202

    response = async_run(client.doc_copy(db_name, "non_existing", "test_new_copy"))
    assert response.status_code == 404
