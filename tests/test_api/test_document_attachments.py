...
def document(client: CouchClient, async_run: Callable):
    ...
    response = async_run(
        client.doc_create_or_update(db_name, doc_name, dict(test=True))
    )
    assert response.status_code == 201
...
def test_exists(client: CouchClient, async_run: Callable):
    response = async_run(client.attachment_exists(db_name, doc_name, attachment_name))
    assert response.status_code == 200

    response = async_run(
        client.attachment_exists(db_name, doc_name, "non_existing_attachment")
    )
    assert response.status_code == 404


def test_get(client: CouchClient, async_run: Callable):
    response = async_run(client.attachment_get(db_name, doc_name, attachment_name))
    assert response.status_code == 200
    assert response.data == attachment_content

    response = async_run(
        client.attachment_get(db_name, doc_name, "non_existing_attachment")
    )
    assert response.status_code == 404
...
def test_delete(client: CouchClient, async_run: Callable):
    ...
    response = async_run(
        client.attachment_delete(db_name, doc_name, "non_existing_attachment")
    )
    assert response.status_code == 404

    response = async_run(
        client.attachment_delete(db_name, doc_name, "%invalid_attachment#")
    )
    assert response.status_code == 400

    response = async_run(client.attachment_delete(db_name, doc_name, attachment_name))
